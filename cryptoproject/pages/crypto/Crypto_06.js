...
import crypto_06_contract from '../../components/crypto/contract/crypto_01_contract';
...
class Crypto_06 extends Component {
    static defaultData = getDefaultCrypto({
        name: cryptoNames.digiByte,
        index: 6
    });

    static defaultProps = {
        marketData: {},
        data: Crypto_06.defaultData
    };
    ...
    static fetchContract(){
        return fetchCryptoContract(crypto_06_contract, Crypto_06.defaultData.index);
    }

    componentDidMount(){
        /*
        Crypto_06.fetchContract().then(response => {
...
const mapStateToProps = (state) => {
    ...
    return {
        data: crypto.filter(data =>
            data.name.toLowerCase() === Crypto_06.defaultData.name.toLowerCase()
        )[0],
...
export const cryptoData06 = Crypto_06.defaultData;

export default connect(mapStateToProps)(Crypto_06);
