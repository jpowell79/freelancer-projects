...
import crypto_15_contract from '../../components/crypto/contract/crypto_01_contract';
...
class Crypto_15 extends Component {
    static defaultData = getDefaultCrypto({
        name: cryptoNames.verge,
        index: 15
    });

    static defaultProps = {
        marketData: {},
        data: Crypto_15.defaultData
    };
    ...
    static fetchContract(){
        return fetchCryptoContract(crypto_15_contract, Crypto_15.defaultData.index);
    }

    componentDidMount(){
        /*
        Crypto_15.fetchContract().then(response => {
...
const mapStateToProps = (state) => {
    ...
    return {
        data: crypto.filter(data =>
            data.name.toLowerCase() === Crypto_15.defaultData.name.toLowerCase()
        )[0],
...
export const cryptoData15 = Crypto_15.defaultData;

export default connect(mapStateToProps)(Crypto_15);
