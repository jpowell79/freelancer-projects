...
import crypto_02_contract from '../../components/crypto/contract/crypto_01_contract';
...
class Crypto_02 extends Component {
    static defaultData = getDefaultCrypto({
        name: cryptoNames.litecoin,
        index: 2
    });

    static defaultProps = {
        marketData: {},
        data: Crypto_02.defaultData
    };
    ...
    static fetchContract(){
        return fetchCryptoContract(crypto_02_contract, Crypto_02.defaultData.index);
    }

    componentDidMount(){
        /*
        Crypto_02.fetchContract().then(response => {
...
const mapStateToProps = (state) => {
    ...
    return {
        data: crypto.filter(data =>
            data.name.toLowerCase() === Crypto_02.defaultData.name.toLowerCase()
        )[0],
...
export const cryptoData02 = Crypto_02.defaultData;

export default connect(mapStateToProps)(Crypto_02);
