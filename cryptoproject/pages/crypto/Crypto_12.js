...
import crypto_12_contract from '../../components/crypto/contract/crypto_01_contract';
...
class Crypto_12 extends Component {
    static defaultData = getDefaultCrypto({
        name: cryptoNames.bitShares,
        index: 12
    });

    static defaultProps = {
        marketData: {},
        data: Crypto_12.defaultData
    };
    ...
    static fetchContract(){
        return fetchCryptoContract(crypto_12_contract, Crypto_12.defaultData.index);
    }

    componentDidMount(){
        /*
        Crypto_12.fetchContract().then(response => {
...
const mapStateToProps = (state) => {
    ...
    return {
        data: crypto.filter(data =>
            data.name.toLowerCase() === Crypto_12.defaultData.name.toLowerCase()
        )[0],
...
export const cryptoData12 = Crypto_12.defaultData;

export default connect(mapStateToProps)(Crypto_12);
