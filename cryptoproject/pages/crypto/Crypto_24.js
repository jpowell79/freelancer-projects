...
import crypto_24_contract from '../../components/crypto/contract/crypto_01_contract';
...
class Crypto_24 extends Component {
    static defaultData = getDefaultCrypto({
        name: cryptoNames.digixDao,
        index: 24
    });

    static defaultProps = {
        marketData: {},
        data: Crypto_24.defaultData
    };
    ...
    static fetchContract(){
        return fetchCryptoContract(crypto_24_contract, Crypto_24.defaultData.index);
    }

    componentDidMount(){
        /*
        Crypto_24.fetchContract().then(response => {
...
const mapStateToProps = (state) => {
    ...
    return {
        data: crypto.filter(data =>
            data.name.toLowerCase() === Crypto_24.defaultData.name.toLowerCase()
        )[0],
...
export const cryptoData24 = Crypto_24.defaultData;

export default connect(mapStateToProps)(Crypto_24);
