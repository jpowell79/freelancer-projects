...
import crypto_23_contract from '../../components/crypto/contract/crypto_23_contract';
...
class Crypto_23 extends Component {
    static defaultData = getDefaultCrypto({index: 23});

    static defaultProps = {
        marketData: {},
        data: Crypto_23.defaultData
    };
    ...
    componentDidMount(){
        Crypto_23.fetchContract().then(response => {
...
const mapStateToProps = (state) => {
    ...
    let cryptoData = crypto.filter(data =>
        data.index === Crypto_23.defaultData.index
    )[0];
...
export const cryptoData23 = Crypto_23.defaultData;

export default connect(mapStateToProps)(Crypto_23);
