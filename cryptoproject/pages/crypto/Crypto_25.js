...
import crypto_25_contract from '../../components/crypto/contract/crypto_01_contract';
...
class Crypto_25 extends Component {
    static defaultData = getDefaultCrypto({
        name: cryptoNames.steem,
        index: 25
    });

    static defaultProps = {
        marketData: {},
        data: Crypto_25.defaultData
    };
    ...
    static fetchContract(){
        return fetchCryptoContract(crypto_25_contract, Crypto_25.defaultData.index);
    }

    componentDidMount(){
        /*
        Crypto_25.fetchContract().then(response => {
...
const mapStateToProps = (state) => {
    ...
    return {
        data: crypto.filter(data =>
            data.name.toLowerCase() === Crypto_25.defaultData.name.toLowerCase()
        )[0],
...
export const cryptoData25 = Crypto_25.defaultData;

export default connect(mapStateToProps)(Crypto_25);
