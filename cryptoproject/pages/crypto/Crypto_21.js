...
import crypto_21_contract from '../../components/crypto/contract/crypto_01_contract';
...
class Crypto_21 extends Component {
    static defaultData = getDefaultCrypto({
        name: cryptoNames.decred,
        index: 21
    });

    static defaultProps = {
        marketData: {},
        data: Crypto_21.defaultData
    };
    ...
    static fetchContract(){
        return fetchCryptoContract(crypto_21_contract, Crypto_21.defaultData.index);
    }

    componentDidMount(){
        /*
        Crypto_21.fetchContract().then(response => {
...
const mapStateToProps = (state) => {
    ...
    return {
        data: crypto.filter(data =>
            data.name.toLowerCase() === Crypto_21.defaultData.name.toLowerCase()
        )[0],
...
export const cryptoData21 = Crypto_21.defaultData;

export default connect(mapStateToProps)(Crypto_21);
