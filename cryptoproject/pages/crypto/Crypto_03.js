...
class Crypto_03 extends Component {
    static defaultData = getDefaultCrypto({
        name: cryptoNames.ripple,
        index: 3
    });

    static defaultProps = {
        marketData: {},
        data: Crypto_03.defaultData
    };
    ...
    static fetchContract(){
        return fetchCryptoContract(crypto_03_contract, Crypto_03.defaultData.index);
    }
...
const mapStateToProps = (state) => {
    ...
    return {
        data: crypto.filter(data =>
            data.name.toLowerCase() === Crypto_03.defaultData.name.toLowerCase()
        )[0],
...
export const cryptoData03 = Crypto_03.defaultData;

export default connect(mapStateToProps)(Crypto_03);
