...
import crypto_18_contract from '../../components/crypto/contract/crypto_18_contract';
...
class Crypto_18 extends Component {
    static defaultData = getDefaultCrypto({
        name: cryptoNames.ethereum,
        index: 18
    });

    static defaultProps = {
        marketData: {},
        data: Crypto_18.defaultData
    };
    ...
    static fetchContract(){
        return fetchCryptoContract(crypto_18_contract, Crypto_18.defaultData.index);
    }

    componentDidMount(){
        Crypto_18.fetchContract().then(response => {
...
const mapStateToProps = (state) => {
    ...
    return {
        data: crypto.filter(data =>
            data.name.toLowerCase() === Crypto_18.defaultData.name.toLowerCase()
        )[0],
...
export const cryptoData18 = Crypto_18.defaultData;

export default connect(mapStateToProps)(Crypto_18);
