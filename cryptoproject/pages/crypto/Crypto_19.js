...
import crypto_19_contract from '../../components/crypto/contract/crypto_01_contract';
...
class Crypto_19 extends Component {
    static defaultData = getDefaultCrypto({
        name: cryptoNames.siacoin,
        index: 19
    });

    static defaultProps = {
        marketData: {},
        data: Crypto_19.defaultData
    };
    ...
    static fetchContract(){
        return fetchCryptoContract(crypto_19_contract, Crypto_19.defaultData.index);
    }

    componentDidMount(){
        /*
        Crypto_19.fetchContract().then(response => {
...
const mapStateToProps = (state) => {
    ...
    return {
        data: crypto.filter(data =>
            data.name.toLowerCase() === Crypto_19.defaultData.name.toLowerCase()
        )[0],
...
export const cryptoData19 = Crypto_19.defaultData;

export default connect(mapStateToProps)(Crypto_19);
