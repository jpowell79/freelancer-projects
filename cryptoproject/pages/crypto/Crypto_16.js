...
import crypto_16_contract from '../../components/crypto/contract/crypto_01_contract';
...
class Crypto_16 extends Component {
    static defaultData = getDefaultCrypto({
        name: cryptoNames.tether,
        index: 16
    });

    static defaultProps = {
        marketData: {},
        data: Crypto_16.defaultData
    };
    ...
    static fetchContract(){
        return fetchCryptoContract(crypto_16_contract, Crypto_16.defaultData.index);
    }

    componentDidMount(){
        /*
        Crypto_16.fetchContract().then(response => {
...
const mapStateToProps = (state) => {
    ...
    return {
        data: crypto.filter(data =>
            data.name.toLowerCase() === Crypto_16.defaultData.name.toLowerCase()
        )[0],
...
export const cryptoData16 = Crypto_16.defaultData;

export default connect(mapStateToProps)(Crypto_16);
