...
import crypto_14_contract from '../../components/crypto/contract/crypto_14_contract';
...
class Crypto_14 extends Component {
    static defaultData = getDefaultCrypto({index: 14});

    static defaultProps = {
        marketData: {},
        data: Crypto_14.defaultData
    };
    ...
    componentDidMount(){
        Crypto_14.fetchContract().then(response => {
...
const mapStateToProps = (state) => {
    ...
    let cryptoData = crypto.filter(data =>
        data.index === Crypto_14.defaultData.index
    )[0];
...
export const cryptoData14 = Crypto_14.defaultData;

export default connect(mapStateToProps)(Crypto_14);
