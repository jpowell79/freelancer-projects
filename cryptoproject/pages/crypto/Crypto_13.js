...
import crypto_13_contract from '../../components/crypto/contract/crypto_13_contract';
...
class Crypto_13 extends Component {
    static defaultData = getDefaultCrypto({
        name: cryptoNames.stellar,
        index: 13
    });

    static defaultProps = {
        marketData: {},
        data: Crypto_13.defaultData
    };
    ...
    static fetchContract(){
        return fetchCryptoContract(crypto_13_contract, Crypto_13.defaultData.index);
    }

    componentDidMount(){
        Crypto_13.fetchContract().then(response => {
...
const mapStateToProps = (state) => {
    ...
    return {
        data: crypto.filter(data =>
            data.name.toLowerCase() === Crypto_13.defaultData.name.toLowerCase()
        )[0],
...
export const cryptoData13 = Crypto_13.defaultData;

export default connect(mapStateToProps)(Crypto_13);
