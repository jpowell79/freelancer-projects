...
import crypto_11_contract from '../../components/crypto/contract/crypto_01_contract';
...
class Crypto_11 extends Component {
    static defaultData = getDefaultCrypto({
        name: cryptoNames.byteCoin,
        index: 11
    });

    static defaultProps = {
        marketData: {},
        data: Crypto_11.defaultData
    };
    ...
    static fetchContract(){
        return fetchCryptoContract(crypto_11_contract, Crypto_11.defaultData.index);
    }

    componentDidMount(){
        /*
        Crypto_11.fetchContract().then(response => {
...
const mapStateToProps = (state) => {
    ...
    return {
        data: crypto.filter(data =>
            data.name.toLowerCase() === Crypto_11.defaultData.name.toLowerCase()
        )[0],
...
export const cryptoData11 = Crypto_11.defaultData;

export default connect(mapStateToProps)(Crypto_11);
