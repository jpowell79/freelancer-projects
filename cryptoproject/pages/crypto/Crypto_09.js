...
import crypto_09_contract from '../../components/crypto/contract/crypto_09_contract';
...
class Crypto_09 extends Component {
    static defaultData = getDefaultCrypto({index: 9});

    static defaultProps = {
        marketData: {},
        data: Crypto_09.defaultData
    };
    ...
    componentDidMount(){
        Crypto_09.fetchContract().then(response => {
...
const mapStateToProps = (state) => {
    ...
    let cryptoData = crypto.filter(data =>
        data.index === Crypto_09.defaultData.index
    )[0];
...
export const cryptoData09 = Crypto_09.defaultData;

export default connect(mapStateToProps)(Crypto_09);
