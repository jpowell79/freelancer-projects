...
import crypto_10_contract from '../../components/crypto/contract/crypto_10_contract';
...
class Crypto_10 extends Component {
    static defaultData = getDefaultCrypto({
        name: cryptoNames.monero,
        index: 10
    });

    static defaultProps = {
        marketData: {},
        data: Crypto_10.defaultData
    };
    ...
    static fetchContract(){
        return fetchCryptoContract(crypto_10_contract, Crypto_10.defaultData.index);
    }

    componentDidMount(){
        Crypto_10.fetchContract().then(response => {
...
const mapStateToProps = (state) => {
    ...
    return {
        data: crypto.filter(data =>
            data.name.toLowerCase() === Crypto_10.defaultData.name.toLowerCase()
        )[0],
...
export const cryptoData10 = Crypto_10.defaultData;

export default connect(mapStateToProps)(Crypto_10);
